...
div.stButton > button {
    background-color: #151820; color: #94a3b8; border: 1px solid #1e2130;
    border-radius: 8px; font-weight: 500; font-size: 15px;
    padding: 10px 24px; width: 100%; transition: all 0.3s ease;
}
div.stButton > button:hover {
    background-color: rgba(74,222,128,0.05); border-color: #4ade80;
    color: #4ade80; transform: translateY(-2px);
    box-shadow: 0 4px 15px rgba(74,222,128,0.15);
}
div.stButton > button:active { transform: translateY(0px); border-color: #22c55e; color: #22c55e; }
div[data-baseweb="input"] > div, div[data-baseweb="select"] > div {
    background-color: #111318 !important; border: 1px solid #2a3040 !important;
    border-radius: 8px !important; transition: all 0.3s ease !important;
}
div[data-baseweb="input"] > div:focus-within, div[data-baseweb="select"] > div:focus-within {
    border-color: #4ade80 !important; box-shadow: 0 0 10px rgba(74,222,128,0.2) !important;
    background-color: #151820 !important;
}
div[data-baseweb="input"] input, div[data-baseweb="select"] div {
    color: #f0f2f8 !important; -webkit-text-fill-color: #f0f2f8 !important;
}
.analysis-box {
    background: linear-gradient(145deg, #111620 0%, #0d0f14 100%);
    border: 1px solid #1e3040; border-left: 3px solid #4ade80;
    border-radius: 10px; padding: 20px 24px; font-size: 14px;
    line-height: 1.9; color: #c8d0e0; white-space: pre-wrap;
...
def cross_validate(df_daily, df_weekly):
    ...
    try:
        ...
        ichi_score = max(0, min(25, ichi_score)); score += ichi_score
    ...
    max_score += 20
    mtf_score = 0; mtf_label = "주봉 데이터 없음"
    ...
    max_score += 15
    fib_score = 0; fib_label = "지지구간 미확인"
    try:
...
=== 분석 요청 ===
1. 🌊 엘리어트 파동 위치 — 현재 파동 위치와 근거, 다음 시나리오
2. ☁️ 일목균형표 해석 — 구름 위치, 전환/기준선, 지지/저항
3. 📅 멀티타임프레임 — 주봉 vs 일봉 방향 일치 여부
4. 📐 피보나치 매매 전략 — 진입가, 손절가(ATR={ind['atr']:,.0f}), 1차/2차 목표가
5. ✅ 최종 판단 — 신뢰도 {cv_result['confidence']}% 기반 매수/매도/관망, 파동 무효화 조건
"""
...
def score_stock(name, ticker):
    try:
        ...
        return dict(
            ...
            buy_count=cv["buy_count"], cur=close, atr=atr,
...
def build_screening_chart(r):
    df = r["df"]; close = df["Close"].squeeze()
    ...
    if "61.8%" in ret:
        fig.add_hline(y=ret["61.8%"], line_dash="dot", line_color="#4ade80", line_width=0.8, annotation_text=f" Fib 61.8% {ret['61.8%']:,.0f}", annotation_font_color="#4ade80", annotation_font_size=9)
...
# ══════════════════════════════════════
# 세션 초기화  ← 오류3 수정: selected_sector 추가
# ══════════════════════════════════════
...
<div class="time-panel">
    ...
    <div class="row"><span>현재 시간</span><span class="clock" id="live-clock">--:--:--</span></div>
    <div class="row"><span>마지막 업데이트</span><span class="fetch">{fetch_time_str}</span></div>
    <div class="row" style="margin-bottom:0;"><span>다음 업데이트</span><span class="next">{next_update_str}</span></div>
</div>
<script>
...
function updateTime() {{
    ...
    if (now.getTime() >= targetTime + 3000) {{ window.parent.location.reload(); }}
}}
...
# ══════════════════════════════════════
    ...
    if df_d is None or len(df_d) < 30:
        ...
        conf = cv["confidence"]; conf_color = cv["verdict_color"]
        st.markdown(f"""
<div style='background:#151820;border:2px solid {conf_color};border-radius:14px;padding:20px 28px;margin-bottom:20px;'>
  ...
  <div style='background:#1a1e2a;border-radius:4px;height:8px;margin-top:12px;'>
    <div style='background:{conf_color};height:8px;border-radius:4px;width:{conf}%;'></div>
  </div>
</div>""", unsafe_allow_html=True)
        ...
        direction_colors = {
            "매수":"#4ade80","강한매수":"#4ade80","추세추종매수":"#86efac",
...
<span style='color:#4ade80;font-weight:700;'>■ 75% 이상</span> : 5개 지표 일치 — 완벽한 유니콘 타점<br>
<span style='color:#86efac;font-weight:700;'>■ 60~74% (주력 매수 구간)</span> : 안정적 분할매수 최적기<br>
<span style='color:#fbbf24;font-weight:700;'>■ 45~59% (스나이퍼 구간)</span> : 핵심 지지선 도달 시 바닥 타점
</div>""", unsafe_allow_html=True)
