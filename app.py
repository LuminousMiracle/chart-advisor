...
# 오류1: st.markdown이 두 번 중복 선언되어 있고 따옴표가 잘못됨 → 하나로 통합
st.markdown("""
<style>
...
def cross_validate(df_daily, df_weekly):
    ...
    if confidence >= 75 and buy_count >= 3:
        verdict, verdict_color = "강한 매수", "#4ade80"
    elif confidence >= 60 and buy_count >= 2:
        verdict, verdict_color = "매수", "#86efac"
    elif confidence >= 45:
        verdict, verdict_color = "관망→매수준비", "#fbbf24"
    elif confidence < 30:
        verdict, verdict_color = "관망/매도검토", "#f87171"
    else:
        verdict, verdict_color = "관망", "#94a3b8"
...
def build_chart(df, ticker, show_fib=True, show_ichi=True):
    fig = make_subplots(
        ...
        subplot_titles=["", "RSI + StochRSI", "MACD", "거래량"]
    )
    close = df["Close"].squeeze()
    high  = df["High"].squeeze()
    low   = df["Low"].squeeze()
    ...
    # 오류2: update_layout에 닫는 괄호가 두 번 중복 '))'로 끝남 → 하나로 수정
    fig.update_layout(
        paper_bgcolor="#0d0f14", plot_bgcolor="#0d0f14",
        font=dict(color="#5a6070", size=10),
        xaxis_rangeslider_visible=False, height=760,
        margin=dict(l=0, r=110, t=20, b=0),
        legend=dict(bgcolor="#111318", bordercolor="#1e2130", borderwidth=1,
                    orientation="h", x=0, y=1.01, font=dict(size=9)),
        hovermode="x unified",
...
=== 분석 요청 ===
1. 🌊 엘리어트 파동 위치 — 현재 파동 위치와 근거, 다음 예상 시나리오
2. ☁️ 일목균형표 해석 — 구름 위치, 전환/기준선, 지지/저항 레벨
...
def build_screening_chart(r):
    ...
    if "61.8%" in ret:
        fig.add_hline(y=ret["61.8%"], line_dash="dot", line_color="#4ade80", line_width=0.8,
                      annotation_text=f" Fib 61.8% {ret['61.8%']:,.0f}",
    ...
    fig.add_hline(y=r["target_buy"],  line_color="#4ade80", line_width=2, line_dash="dash",
                  annotation_text=f"  매수가 {r['target_buy']:,.0f}",
                  annotation_position="right", annotation_font_color="#4ade80")
    fig.add_hline(y=r["target_sell"], line_color="#f87171", line_width=2, line_dash="dash",
                  annotation_text=f"  목표가 {r['target_sell']:,.0f}",
                  annotation_position="right", annotation_font_color="#f87171")
    fig.update_layout(paper_bgcolor="#0d0f14", plot_bgcolor="#0d0f14",
                      ...
                      legend=dict(bgcolor="#111318",bordercolor="#1e2130",borderwidth=1,
                                  orientation="h",x=0,y=1.08,font=dict(size=9)))
...
if st.session_state.mode == "home":
  ...
  <div><span style='font-size:10px;color:#fbbf24;margin-right:8px;'>⚡ 다크호스</span>{db}</div>
    ...
    if indices:
        ...
        for col,(name,data) in zip(cols,indices.items()):
            ...
            col.markdown(f"""<div style='background:#151820;border:1px solid #1e2130;border-radius:10px;padding:14px 10px;text-align:center;'>
  ...
  <div style='font-size:12px;color:{color};font-weight:600;'>{sign}{abs(chg):.2f}%</div>
    ...
    if indices:
        ...
        with c1:
            st.markdown(f"""<div style='background:#151820;border:1px solid #1e2130;border-radius:12px;padding:20px;text-align:center;'>
  ...
  <div style='background:#1a1e2a;border-radius:4px;height:8px;margin-top:12px;'>
    <div style='background:{conf_color};height:8px;border-radius:4px;width:{conf}%;'></div>
        ...
        for col, sig in zip(sig_cols, cv["signals"]):
            ...
            col.markdown(f"""<div style='background:#151820;border:1px solid #1e2130;border-left:3px solid {dc};border-radius:8px;padding:12px;height:140px;overflow:hidden;'>
...
# ══════════════════════════════════════
    ...
    if st.session_state.screen_result is None:
        ...
        for i,r in enumerate(st.session_state.screen_result):
            ...
            st.markdown(f"""<div style='background:#151820;border:2px solid {conf_color};border-radius:12px;padding:16px 20px;margin-bottom:16px;'>
